...
const DragHandle = SortableHandle(() => <span className="pointer">::</span>);
  ...
  ({
    ...
    return (
      <div key={task.id} className="task-card" id={`task-${task.id}`}>
        <div style={{ marginBottom: 20 }} className="d-flex justify-between">
          <div>
            <input
              type="text"
          ...
          <div className="d-flex justify-center align-center"><DragHandle /></div>
        </div>
...
const TaskList = ({
  ...
  const [cardPicked, setCardPicked] = useState(-1);
  useEffect(() => {
    if (checkDnd) {
      setTheTasks(tasks);
      setCheckDnd(0);
  ...
  const handleSortEnd = ({ oldIndex, newIndex }: IIndex) => {
    ...
    const task = tasks[cardPicked];
    const taskDiv: Element | null = document.querySelector(`#task-${task.id}`);
    
    if (taskDiv) {
       taskDiv.classList.remove('task-card-box-shadow');
    }
    setCardPicked(-1);
  ...
  const updateBeforeSortStartHandler = (result: SortStart) => {
    setCardPicked(result.index);
    const taskToSort = tasks[result.index];
    const taskDiv: Element | null = document.querySelector(`#task-${taskToSort.id}`);
    if (taskDiv) {
       taskDiv.classList.add('task-card-box-shadow');
    }
  }
  return (
    <div>
      {tasks.length > 0 && (
        <SortableList
          ...
          axis="xy"
          useDragHandle
          updateBeforeSortStart={updateBeforeSortStartHandler}
        />
      )}
    </div>
  );
